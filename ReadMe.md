...
Progresses are created and modified by users. It's natural to model this relationship with a [ancestor relationship](https://cloud.google.com/appengine/docs/python/ndb/entities).

#### Keys

We will use the MD5 hashed E-Mail address as the key for `User`. Although, this bears the small chance of generating duplicate keys, it provides some benefits when dealing with API keys as we will see in the [Authentication](#Authentication) section. Also, note that OpenID connect protocol  provides a unique user id that could be used instead.

We let the datastore automatically generate unique ids used as keys for `Progress` entities. Note that progresses are created by users, and we therefore will require that a `Progress` key needs to have a `User` key as parent. This also means that the unique ids generated by the datastore will only be unique in the context of a parent `User` key.
...
`progressApi` supports OpenID Connect for user authentication. Once a user authenticates the user fetched from the `User` entity. If the user is not found in the datastore, a new `User` will be created on the fly. This way, sign up and sign in are actually one step.
...
The API key is randomly generated id that can be passed along each `progressApi` request. If the backend API detects that an API key has been passed along with a request, it looks up the corresponding `User` and verifies that the two Api keys match.
